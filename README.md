...
1. EDA (programmatic via `src/eda.py`)
...
│   ├── eda.py                  # programmatic EDA (tables)
...
│   ├── sweep.py                # run_full_sweep() — single top-level Open Source SW Contribution function
...
| `--run-sweep` | off | Run the single top-level full sweep (Open Source SW Contribution) — scaler × encoder × model × params, writes `reports/full_sweep*.{csv,json}` and `figures/full_sweep_top5.png` |
| `--sweep-cv N` | `3` | Folds for the full sweep |
| `--sweep-sample N` | `20000` | Subsample size for the full sweep (0 = full dataset) |
| `--all` | off | Enable EDA + all ablations + auxiliary tasks + full sweep |
